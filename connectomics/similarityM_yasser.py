...
def __dilate(img,new_shape = (193, 229, 193)):
    # Calculate the zoom factors for each dimension
    data = np.nan_to_num(img.get_fdata())
    data = (data - data.min()) / (data.max() - data.min())
    img  = nib.nifti1.Nifti1Image(data, img.affine,header=img.header)
    target_affine = np.eye(4)
    target_affine[0, 0] = img.header.get_zooms()[0] * img.shape[0] / new_shape[0]
    target_affine[1, 1] = img.header.get_zooms()[1] * img.shape[1] / new_shape[1]
    target_affine[2, 2] = img.header.get_zooms()[2] * img.shape[2] / new_shape[2]
    resampled_img       = resample_img(img, target_affine=target_affine, target_shape=new_shape,)
...
def bundles_to_connectivity(hfName, outBasename, scaleId, mapFilename, voxth=0, subth=0, boolbund=False, boolforce=False):
    ...
    # Reading streamlines connecting each pair of gray matter regions.
    numbStlines      = np.array(hf.get('matrices/numbStlines'))

    # Mean length of the streamlines connecting each pair of gray matter regions.
    lengthStlines      = np.array(hf.get('matrices/length'))
    ...
    if os.path.isfile(mapFilename):
        mapI    = nib.load(mapFilename)
        dim_og  = mapI.get_fdata().shape
        if np.all(dim_og[0:3]!=dim):
            mapI    = dilate(mapI,new_shape = dim)
            dataM   = mapI.get_fdata()
            print("Dilated dataM shape from",dim_og,"to",dataM.shape)
        else:
            dataM   = mapI.get_fdata()
            print("Data has already matching shape with atlas",dataM.shape)
        
        booltab = True

    
    # dataM = np.random.rand(193, 229, 193, 2)*255.0

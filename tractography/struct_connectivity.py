...
subject_id,session = sys.argv[1],sys.argv[2]
recording = subject_id,session
debug.title(f"Processing sub-{subject_id}ses-{session}")
debug.separator()
...
mrsi_nii        = mridata.data["mrsi"]["CrPCr"]["orig"]["path"]
...
if mrsi_nii==0: 
    ...
    progress_dict[subject_id][session]["progress"]=f"Skip: No MRSI data found"
    tctools.exit_run(dict_path,progress_dict)
